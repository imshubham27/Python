...
for item in all1:
    ...
    try:
        d["Release Date"] = (
            item.find("h4", {"class": "li_group"}).find("a").text)
    except:
    ...
    try:
        d["Directors"] = (
            item.find("div", {"class": "txt-block"}).find("span").find("a").text)
    except:
        d["Directors"] = "Not mentioned"
    try:
        d["Running Time"] = (
            item.find("p", {"class": "cert-runtime-genre"}).find("time").text)
    except:
...
for i in range(dt.now().month+1, 13, 1):
    print("https://www.imdb.com/movies-coming-soon/2021-" +
          f"{i:02d}"+"/?ref_=cs_dt_nx")
    web_url = requests.get(
        "https://www.imdb.com/movies-coming-soon/2021-"+f"{i:02d}"+"/?ref_=cs_dt_nx").text
